                                ...
                                <div class="rounded border p-1 my-2" style="background-color: #537429; box-shadow: 2px 2px 4px #000000;">
                                    @foreach($filiale->products as $item)
                                        <div class="row justify-between my-1 align-items-center">
                                            ...
                                            <div class="col-3">
                                                <input wire:model.defer="matricola.{{$item->id}}" type="text" style="color: black" class="w-full rounded border shadow p-2 mr-2 my-2" placeholder="Matricola">
                                            </div>
                                            <div class="col-4">
                                                <a wire:click.prevent="aggiungiAlDdt({{$item}}, {{$filiale->id}})" class="btn btn-success">aggiungi al DDT</a>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        @foreach($ddt as $key => $item)
                            <div class="rounded border p-1 my-2 mr-4" style="background-color: #052e3c; box-shadow: 2px 2px 4px #000000; color: white">
                                <div class="row justify-between my-1 align-items-center">
                                    ...
                                    <div class="col-2">
                                        <i class="fas fa-times text-red-200 hover:text-red-600 cursor-pointer" wire:click.prevent="removeFromDdt({{$key}})"></i>
                                    </div>
                                </div>
                            </div>
                        @endforeach

                        @if(count($ddt) > 0)   <a wire:click.prevent="$emit('produciDdt')" class="btn btn-success mt-5">produci DDT</a> @endif
